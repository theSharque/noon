<script>
  ...
  function altitudeLabel(p) {
    if (p < 0.2) return 'орбита';
    if (p < 0.45) return 'вход в атмосферу';
    if (p < 0.75) return 'снижение';
    if (p < 0.92) return 'у поверхности';
    return 'касание';
  }
  ...
  function ensureField(type, name) {
    ...
    fieldKey = key;
    field = createLandField(type, (Number(type) * 9973 + (seedFromName(name) || 1)) >>> 0);
  }
  ...
  function frame(ts) {
    ...
    if (!field) return;
    stepLandField(field, dt, journeyProgress);
    drawLandField(ctx, field, { w, h }, journeyProgress, ts / 1000);
...
>
  <canvas bind:this={canvas} class="land-canvas"></canvas>
  <div class="hud">
    <div class="hud-title">Посадка{planetName ? ` · ${planetName}` : ''}</div>
