...
if( count( $_POST ) > 0 ) {
  ...
  if( $good ) {
    ...
    db_query( "INSERT INTO users ( login, parent_id, ref_link, ent_link, password, email, ssid, status, place_id, place_type, money, credits, register_date )
                          VALUES ( '$login', $refid, '$reflink', '$entlink', '".md5($pass1)."', '$email', '$tmp', 2, 1, 1, 15000, 0, NOW() )" );
