<script>
  ...
  async function onGoodsChange() {
    if (busy || mode !== 'station' || !goodsValue) return;
    busy = true;
    errorText = '';
    playSelect();
    try {
  ...
  async function onLotRow(idx) {
    if (busy || mode !== 'station' || idx < 0 || idx >= lots.length) return;
    const row = lots[idx];
    lotIdx = idx;
  ...
  onMount(() => {
    ...
    (async () => {
      const [probe, info] = await Promise.all([loadStationGoods(filterRf), getUserInfo()]);
      if (!probe.ok) {
        mode = 'stub';
        return;
      }
...
{:else if mode === 'stub'}
  <div class="stub scifi-panel">
    ...
    <div class="panel-content">
      ...
      <p class="note">Сейчас вы не на станции. Планета, орбита и переходы — следующие экраны.</p>
