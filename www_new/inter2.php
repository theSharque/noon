<?php
  ...
  if( $_POST['ik_sign_hash'] === $sign_hash && $_POST['ik_payment_state'] == 'success' ) {
...
//    debug_it( 'Success' );
    ...
    if( $check == $id ) {
      ...
      db_query( "INSERT INTO pay_log ( user_id, type, mess ) VALUES ( $uid, 1, 'Success pay of $sum ID:$id' )" );

      $vals = serialize( $_POST );
      db_query( "INSERT INTO 2pay_pays ( output, user_id ) VALUES ( '$vals', $uid )" );

      $result = db_fetch_row( "SELECT id, donate, meta_in, relid, login, substr( register_date, 1, 10 ) reg_date FROM users WHERE id = $uid" );
