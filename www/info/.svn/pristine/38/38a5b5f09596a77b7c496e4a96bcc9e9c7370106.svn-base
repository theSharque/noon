	...
	<?php } if ($_forumrow_val['S_IS_CAT'] || $_forumrow_val['S_FIRST_ROW'] || $_forumrow_val['S_NO_CAT']) {  ?>
		...
		<div class="forabg" id="forabg-<?php echo $_forumrow_val['FORUM_ID']; ?>">
			<?php echo (isset($this->_tpldata['DEFINE']['.']['CA_BLOCK2_START'])) ? $this->_tpldata['DEFINE']['.']['CA_BLOCK2_START'] : ''; ?>
    			...
    			<?php if ($_forumrow_val['S_IS_CAT']) {  ?><h2><a href="<?php echo $_forumrow_val['U_VIEWFORUM']; ?>"><?php echo $_forumrow_val['FORUM_NAME']; ?></a></h2><?php } else { echo ((isset($this->_rootref['L_FORUM'])) ? $this->_rootref['L_FORUM'] : ((isset($user->lang['FORUM'])) ? $user->lang['FORUM'] : '{ FORUM }')); } ?>

            <?php echo (isset($this->_tpldata['DEFINE']['.']['CA_BLOCK2_START2'])) ? $this->_tpldata['DEFINE']['.']['CA_BLOCK2_START2'] : ''; ?>    			
			<ul class="topiclist">
				<li class="header">
					<dl class="icon" id="expand-<?php echo $_forumrow_val['FORUM_ID']; ?>">
		...
		<li class="row<?php if ($_forumrow_val['S_UNREAD_FORUM']) {  ?> row-new<?php } ?>" id="forumrow-<?php echo $_forumrow_val['FORUM_ID']; ?>">
			<dl class="icon" style="background-image: url(<?php echo $_forumrow_val['FORUM_FOLDER_IMG_SRC']; ?>); background-repeat: no-repeat;">
				<dt>
					...
					<?php } if ($_forumrow_val['SUBFORUMS'] && $_forumrow_val['S_LIST_SUBFORUMS']) {  ?><br /><strong><?php echo $_forumrow_val['L_SUBFORUM_STR']; ?></strong> <?php echo $_forumrow_val['SUBFORUMS']; } if ($_forumrow_val['FORUM_IMAGE']) {  ?></span><?php } ?>
