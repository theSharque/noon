...
//			pLoader.load(new URLRequest("d:\page311.txt"));
		...
		private function placeChange(event:Event) {
			sLoader.load(new URLRequest("page.php?id=31&oth="+cbFilter.selected+"&pl="+cbPlace.selectedItem.data));
		...
		public function shipsLoaded(event:Event) {
			...
			if (sLoader.data.cnt > 0) {
				for (i=0; i<sLoader.data.cnt; i++) {
					if (dgShips.selectedIndex == 0 && lastShip == sLoader.data["id" + i]) {
		...
		public function shipsRefresh(e:TimerEvent) {
			...
			if (sLoader.data.cnt > 0) {
				...
				for (i=0; i<sLoader.data.cnt; i++) {
					...
					if( aPos.length == 1 && aPos[0] == i && sLoader.data["c" + i] != '0xCC0000' ) {
						needScroll = true;
					}
		...
		public function fireRender( event:Event ) {
			...
			if( wLoader.data.err == 0 && wLoader.data.cnt > 0 ) {
				...
				for( i = 0; i < wLoader.data.cnt; i++ ) {
					...
					if( die ) {
						var mvBoom:MovieClip = new boom();
						mvBoom.x = dObj.x;
						mvBoom.y = dObj.y;
						mvBoom.height = dObj.height;
						mvBoom.width = dObj.height;

						dTween = new Tween(dObj, "alpha", Strong.easeOut, 100, 0, 1, true);
						bTween = new Tween(mvBoom, "alpha", Strong.easeOut, 100, 0, 1, true);
					}
				}
		...
		public function clickOrder(e:MouseEvent) {
			...
			switch ( int( id ) ) {
				...
				case 41 :
					...
					break;
				case 11 :
					btUrl="page.php?id=33&shid="+sLoader.data["id"+dgShips.selectedIndex]+"&orid=11&nname="+monitor.dtName.text+"&gname="+monitor.dtGroup.text;
					nopLoader.load(new URLRequest( btUrl ));
					//nopLoader.load(new URLRequest("d:\page33.txt"));
					break;
				case 14 :
